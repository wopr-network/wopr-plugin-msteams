 ...
 */
export function registerMsteamsTools(
	...
	registry.register({
		...
		description:
			"Get MS Teams bot connection status: online/offline, connected tenants, and uptime.",
		parameters: {},
	...
	registry.register({
		name: "listTeams",
		description:
			"List connected MS Teams organizations the bot has interacted with.",
		parameters: {},
		handler: async (_params: Record<string, unknown>, auth: AuthContext) => {
	...
	registry.register({
		...
		description:
			"List MS Teams channels the bot is active in, optionally filtered by team.",
		parameters: {
			teamId: {
				type: "string",
		...
		handler: async (params: Record<string, unknown>, auth: AuthContext) => {
			...
			if (teamId) {
				return daemonRequest(
					apiBase,
					`/plugins/msteams/teams/${encodeURIComponent(teamId)}/channels`,
					auth,
				);
			}
	...
	registry.register({
		...
		description:
			"Get MS Teams message processing statistics: messages processed and active conversations.",
		parameters: {},
		handler: async (_params: Record<string, unknown>, auth: AuthContext) => {
